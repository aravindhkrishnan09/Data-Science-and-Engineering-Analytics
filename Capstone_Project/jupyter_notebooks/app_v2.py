        ...
        if not models or not scaler:
            ...
            if len(X_seq) == 0:
                ...
                with tab_res_soh:
                    ...
                    # Matplotlib fallback for detailed labels
                    fig, ax = plt.subplots(figsize=(5, 3))
                    ax.plot(cycles, preds, label='Predicted SoH', color='blue')
                    ax.set_xlabel("Cycle")
                    ax.set_ylabel("Predicted SoH")
                    ax.set_title(f"{dl_model_choice} Prediction")
                    ax.grid(True)
                    st.pyplot(fig)
                ...
                with tab_res_rul:
                    ...
                    c1, c2 = st.columns(2)
                    c1.metric("Latest Estimated RUL", f"{latest_rul} Cycles")
                    c2.metric("RUL Percentage", f"{min(100.0, max(0.0, (latest_rul / 1000) * 100)):.1f}%")
